...
def _default_profile_dir() -> Path:
    explicit = os.environ.get("poco_PROFILE_PATH", "").strip() or os.environ.get("USER_DATA_DIR", "").strip()
    if explicit:
...
def resolve_profile_dir(value: str | None) -> Path:
    ...
    env_value = os.environ.get("poco_PROFILE_PATH", "").strip() or os.environ.get("USER_DATA_DIR", "").strip()
    if env_value:
...
def preferred_binary_path() -> Path:
    env_value = os.environ.get("poco_CHROMIUM_BINARY", "").strip() or os.environ.get("CHROMIUM_PATH", "").strip()
    if env_value:
