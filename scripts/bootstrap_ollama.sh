...
BOOT_TIMEOUT_SECONDS="${poco_OLLAMA_BOOT_TIMEOUT_SECONDS:-90}"
STRICT_BOOTSTRAP="${poco_OLLAMA_BOOTSTRAP_STRICT:-0}"
PREFER_LOCAL="${poco_PREFER_LOCAL_OLLAMA:-1}"
...
if [[ "${PREFER_LOCAL}" == "1" && "${poco_ENABLE_LOCAL_OLLAMA:-1}" == "1" ]]; then
  unset OLLAMA_HOST
fi
...
if [[ "${poco_ENABLE_LOCAL_OLLAMA:-1}" != "1" ]]; then
...
seen = set()
sources = [
    os.environ.get("poco_BOOTSTRAP_MODELS", ""),
    os.environ.get("poco_OLLAMA_MODELS", ""),
    os.environ.get("poco_PRIMARY_MODEL", ""),
    os.environ.get("poco_MODEL_DIRECTOR", ""),
    os.environ.get("poco_MODEL_TREND", ""),
    os.environ.get("poco_MODEL_CREATOR", ""),
    os.environ.get("poco_MODEL_REPHRASE", ""),
    os.environ.get("poco_MODEL_SELECTOR", ""),
    os.environ.get("poco_MODEL_CODING", ""),
    os.environ.get("poco_MODEL_SUMMARY", ""),
    os.environ.get("poco_MODEL_QUICK", ""),
]
for raw in sources:
